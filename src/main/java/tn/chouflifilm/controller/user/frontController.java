...
User user= userService.recherparid(UserSessionManager.getInstance().getCurrentUser().getId());
    ...
    // Handler for the Backoffice button
    @FXML
    public void openBackoffice() throws IOException {
        // Load the backoffice FXML (adjust path if needed)
        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/com/example/demo/backback.fxml"));
