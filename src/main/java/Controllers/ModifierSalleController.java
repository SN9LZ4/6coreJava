...
public class ModifierSalleController {
    ...
    @FXML private Button modifierSalleButton;
        ...
        try {
            ...
            filmListView.setCellFactory(param -> new ListCell<>() {
                ...
                protected void updateItem(Film film, boolean empty) {
                    ...
                    setText((empty || film == null) ? null : film.getTitre() + " - " + film.getGenre() + " (" + film.getDuree() + ")");
                }
            });
        } catch (Exception e) {
            System.err.println("Erreur chargement films: " + e.getMessage());
        }
    }
    ...
    public void setSalle(Salle salle) {
        ...
        nbrPlacesSpinner.getValueFactory().setValue(salle.getNbr_places());
        typeSalleCombo.setValue(salle.getType_salle());
        etatSalleCombo.setValue(salle.getEtat_salle());
        imageSalleField.setText(salle.getImage_salle());

        if (salle.getImage_salle() != null && !salle.getImage_salle().isEmpty()) {
            try {
                salleImageView.setImage(new Image(salle.getImage_salle()));
            } catch (Exception e) {
                salleImageView.setImage(null);
            }
        }

        Set<Film> films = salle.getFilms();
        if (films != null) {
            filmListView.getSelectionModel().clearSelection();
            for (Film f : films) {
                filmListView.getSelectionModel().select(f);
            }
        }
    }
    ...
    void modifierSalle(ActionEvent event) {
        ...
        if (!nom.matches("^[a-zA-ZÀ-ÿ\\s\\-\\.']{3,100}$")) {
